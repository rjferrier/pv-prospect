...
def test_step2_covers_trailing_14_days() -> None:
    ...
    assert manifest.step2_batch.start_date == _TODAY - timedelta(
        days=WEATHER_GRID_BACKFILL_WINDOW_DAYS
    )


def test_step3_produces_requested_batch_count() -> None:
    cursor = initial_weather_grid_backfill_cursor(_TODAY)
    manifest, _ = build_weather_grid_manifest(
...
def test_step3_batches_march_backwards_in_14_day_windows() -> None:
    ...
    for i, batch in enumerate(manifest.step3_batches):
        assert batch.end_date - batch.start_date == timedelta(
            days=WEATHER_GRID_BACKFILL_WINDOW_DAYS
        )
        if i > 0:
...
def test_updated_cursor_continues_from_last_batch() -> None:
    cursor = initial_weather_grid_backfill_cursor(_TODAY)
    _, updated_cursor = build_weather_grid_manifest(_TODAY, _NUM_SAMPLE_FILES, cursor)
...
def test_consecutive_days_produce_contiguous_step3_batches() -> None:
    ...
    _, cursor = build_weather_grid_manifest(day1, _NUM_SAMPLE_FILES, cursor)

    day2 = day1 + timedelta(days=1)
    manifest2, _ = build_weather_grid_manifest(day2, _NUM_SAMPLE_FILES, cursor)

    # Day 2's first Step 3 batch should start right where day 1 left off
    manifest1, _ = build_weather_grid_manifest(
        day1, _NUM_SAMPLE_FILES, initial_weather_grid_backfill_cursor(day1)
    )
...
def test_sample_file_indices_wrap_around() -> None:
    ...
    cursor = WeatherGridBackfillCursor(
        next_end_date=_TODAY - timedelta(days=WEATHER_GRID_BACKFILL_WINDOW_DAYS),
...
def test_initial_cursor_starts_after_step2_window() -> None:
    ...
    assert cursor.next_end_date == _TODAY - timedelta(
        days=WEATHER_GRID_BACKFILL_WINDOW_DAYS
    )
