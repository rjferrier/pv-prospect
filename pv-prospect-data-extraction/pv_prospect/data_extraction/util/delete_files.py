...
def list_files_recursive(
    ...
    files = client.search(
        search_path, mime_type=mime_type, include_trashed=include_trashed
    )
    ...
    for folder in folders:
        ...
        files_with_paths.extend(
            list_files_recursive(
                client, folder['id'], mime_type, folder_path, include_trashed
            )
        )
...
def restore_files(
    ...
    files = list_files_recursive(
        client, root_folder_id, mime_type, include_trashed=True
    )
...
if __name__ == '__main__':
    ...
    parser.add_argument('pattern', help='Regex pattern to match against file paths')
    parser.add_argument(
    ...
    if args.restore:
        # Restore mode
        restore_files(args.folder_path, args.pattern, args.mime_type, args.dry_run)
    else:
        # Delete mode
        # Safety check: require --dry-run or explicit confirmation for permanent deletion
        if args.permanent and not args.dry_run:
            print(
                'WARNING: You are about to PERMANENTLY delete files. This cannot be undone!'
            )
        ...
        delete_files(
            args.folder_path, args.pattern, args.mime_type, args.dry_run, args.permanent
        )
