...
_WORKFLOW = 'pv-prospect-transform'
...
def test_pv_site_weather_input_emits_clean_weather_only() -> None:
    """A weather input is PV-site weather: it is cleaned for the
    prepare_pv join but carries no prepare or assemble of its own."""
    transform_input = TransformInput(
...
def test_weather_input_located_by_location_is_clean_only() -> None:
    """An ad-hoc location-keyed weather input is also clean-only."""
    transform_input = TransformInput(
...
def test_each_input_carries_its_own_window() -> None:
    transform_inputs = [
        TransformInput(DataSourceType.PV, '2026-01-01', '2026-01-29', pv_system_id=1),
        TransformInput(DataSourceType.PV, '2026-02-01', '2026-02-28', pv_system_id=2),
    ]
...
def test_end_date_omitted_when_input_has_none() -> None:
    ...
    phases = build_transform_phases([transform_input], _WORKFLOW, _RUN_DATE)
...
def test_assemble_pv_emitted_per_system_window() -> None:
    ...
    transform_inputs = [
        ...
        TransformInput(
            DataSourceType.PV, '2026-02-01', '2026-02-28', pv_system_id=4708
        ),
        TransformInput(
