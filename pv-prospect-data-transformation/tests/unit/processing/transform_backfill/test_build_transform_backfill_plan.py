"""Tests for build_transform_backfill_plan."""
...
def test_initial_cursor_starts_at_today() -> None:
    ...
    cursor = initial_transform_backfill_cursor(today)
...
def test_plan_covers_window_days_before_cursor_end() -> None:
    ...
    plan, _ = build_transform_backfill_plan(cursor, window_days=14)
...
def test_next_cursor_advances_by_window_days() -> None:
    ...
    _, next_cursor = build_transform_backfill_plan(cursor, window_days=28)
...
def test_successive_plans_do_not_overlap() -> None:
    ...
    plan_1, cursor_1 = build_transform_backfill_plan(cursor, window_days=28)
    plan_2, _ = build_transform_backfill_plan(cursor_1, window_days=28)
...
def test_successive_plans_cover_contiguous_dates() -> None:
    ...
    plan_1, cursor_1 = build_transform_backfill_plan(cursor, window_days=28)
    plan_2, cursor_2 = build_transform_backfill_plan(cursor_1, window_days=28)
    plan_3, _ = build_transform_backfill_plan(cursor_2, window_days=28)
...
def test_custom_window_days() -> None:
    ...
    plan, next_cursor = build_transform_backfill_plan(cursor, window_days=7)

    assert plan.start_date == date(2026, 4, 29)
    assert plan.end_date == date(2026, 5, 6)
    assert next_cursor.next_end_date == date(2026, 4, 29)
...
def test_first_run_from_initial_cursor() -> None:
    ...
    plan, next_cursor = build_transform_backfill_plan(cursor, window_days=28)

    assert plan == TransformBackfillPlan(
