...
from pv_prospect.common import get_pv_site_by_system_id, build_pv_site_repo
from pv_prospect.common import get_bounding_box_by_location, build_bounding_box_repo
...
def _load_resources(io_helper: GcsIoHelper, raw_bucket: str) -> None:
    ...
    bbox_csv = io_helper._raw_bucket.blob("staging/openmeteo_bounding_boxes.csv")
    if bbox_csv.exists():
        data = bbox_csv.download_as_string().decode('utf-8')
        build_bounding_box_repo(StringIO(data))
...
def main() -> None:
    ...
    if step == 'clean_weather':
        ...
        for blob in blobs:
            if date_str in blob.name and blob.name.endswith('.csv'):
                ...
                if df is not None and not df.empty:
                    ...
                    out_path = blob.name.replace("staging/", "cleaned/").replace(".csv", ".parquet")
        ...
        if df is not None and not df.empty:
            ...
            out_path = in_path.replace("staging/", "cleaned/").replace(".csv", ".parquet")
            io_helper.write_parquet_to_raw(cleaned_df, out_path)
        ...
        for blob in blobs:
            if date_str in blob.name and blob.name.endswith('.parquet'):
                ...
                out_path = blob.name.replace("cleaned/timeseries/openmeteo/quarterhourly/", "")
        ...
        bbox = get_bounding_box_by_location(pv_site.location)
        # Bounding box coords in filenames are formatted by stripping decimal points
        # For simplicity in this demo entrypoint, we list and match the lat/lon.
        # Real implementation would use the exact same formatting as OpenMeteoFileMetadata.
        lat_str = str(bbox.latitude).replace('.', '')
        lon_str = str(bbox.longitude).replace('.', '')
        # Formatting to match _str_to_coordinate logic may vary; here we use wildcard matching
