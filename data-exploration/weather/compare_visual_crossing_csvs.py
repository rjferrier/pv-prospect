...
# Load both files
vc_hourly = pd.read_csv(
    '../datasets/data-1/visualcrossing-hourly_89665.csv', parse_dates=['datetime']
)
vc_quarterly = pd.read_csv(
    '../datasets/data-1/visualcrossing-quarterhourly_89665.csv',
...
comparison = pd.merge(
    vc_hourly, vc_quarterly_hourly, on='datetime', suffixes=('_hourly', '_quarterly')
)
...
def report_mismatches(df, columns_to_compare):
    ...
    for col in columns_to_compare:
        ...
        if hourly_col in df.columns and quarterly_col in df.columns:
            mismatches = df[df[hourly_col] != df[quarterly_col]][
                ['datetime', hourly_col, quarterly_col]
            ]
