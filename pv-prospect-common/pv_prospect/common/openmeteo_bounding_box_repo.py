...
def build_openmeteo_bounding_box_repo(csv_stream: TextIOWrapper) -> None:
    if len(om_bounding_boxes_by_pv_system_id) != 0:
        return
    ...
    for row in reader:
        pv_system_id, bb = _parse_row(row)
        om_bounding_boxes_by_pv_system_id[pv_system_id] = bb


def get_openmeteo_bounding_box_by_pv_system_id(pv_system_id: int) -> BoundingBox:
    return om_bounding_boxes_by_pv_system_id[pv_system_id]
