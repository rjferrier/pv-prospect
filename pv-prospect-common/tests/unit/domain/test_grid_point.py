"""Tests for GridPoint domain model."""
...
def test_bin_floors_fractional_positive_coordinates():
    grid_point = GridPoint(Location.from_coordinates('0.9', '0.0'))

    assert grid_point.bin == '0_0'


def test_bin_floors_at_integer_boundary():
    grid_point = GridPoint(Location.from_coordinates('1.0', '0.0'))

    assert grid_point.bin == '1_0'


def test_bin_floors_negative_coordinate_towards_negative_infinity():
    grid_point = GridPoint(Location.from_coordinates('-0.1', '0.0'))

    assert grid_point.bin == '-1_0'


def test_bin_zero_coordinates():
    grid_point = GridPoint(Location.from_coordinates('0.0', '0.0'))
...
def test_from_id_roundtrips():
    ...
    assert GridPoint.from_id(grid_point.id) == grid_point
