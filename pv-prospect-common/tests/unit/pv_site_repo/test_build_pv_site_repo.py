...
def test_get_all_pv_system_ids_returns_sorted():
    _clear_repo()
    build_pv_site_repo(
...
def test_get_pv_site_by_system_id_returns_site():
    _clear_repo()
    build_pv_site_repo(io.StringIO(f'{CSV_HEADER}\n{SAMPLE_ROW_STR}\n'))
    assert get_pv_site_by_system_id(89665).name == 'Test Site'


def test_get_pv_site_by_unknown_id_raises():
    _clear_repo()
    build_pv_site_repo(io.StringIO(f'{CSV_HEADER}\n{SAMPLE_ROW_STR}\n'))
