...
- [ ] [Data-derived planning for the transformation backfills](briefs/data-derived-transform-planning.md) — plan from *raw-present − prepared-present* over `prepared-batches/` instead of extraction ledger + consumed-through marker; markers deleted, transform ledger demoted to audit; absorbs the transform half of `outage-recovery` (`reports/data-pipeline-retrospective.md` rec 1). Sequence after `version-raw-data`, before `tracking-restructure` / `end-date-semantics`
